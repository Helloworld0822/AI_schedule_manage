...
from .models import Schedule as ScheduleModel, CategoryEnum
...
def create_schedule(item: ScheduleIn, db: Session = Depends(get_db)):
    ...
    db_item = ScheduleModel(start=start_dt, end=end_dt, title=item.title, description=item.description, category=cat)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
